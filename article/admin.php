...
?>
			...
			<div  style="margin-left:10px;position:absolute; margin-left:0px; margin-top:2px;height:29px;" >სულ სტატიების რაოდენობა:<b style="color:#478FED;"><? echo $totali; ?></b>
			</div>
			
			
			<select  style="margin-left:10px;position:absolute; margin-left:-160px; margin-top:-2px; width:150px;height:29px;" name="forma" onchange="location = this.options[this.selectedIndex].value;">
	
 	
<?php
										
								if ($_GET["kategoria"] == "arferi" )
								{
								 echo '<option selected value="admin.php?page=1&kategoria=arferi">ყველა</option>';
								}
								else 
								{
								 echo '<option  value="admin.php?page=1&kategoria=arferi">ყველა</option>';
								} 
								?>
...
require( "categoryclass.php");
  ...
  $results1['articles'] = $data['results'];
  
  
?> <?php foreach (  $results1['articles'] as $statiafor ) { ?>		
								...
								<?php
									...
									if ($_GET["kategoria"] == $statiafor->ID  )
								{
 								 echo "<option  selected value='admin.php?page=1&kategoria=". $statiafor->ID ."'>". $statiafor->satauri  ."</option>";
								}
								else 
								{
 								 echo "<option    value='admin.php?page=1&kategoria=". $statiafor->ID ."'>". $statiafor->satauri  ."</option>";
								} ?> 	
        ...
        <!-- <td style="text-align:center;">  <?php echo   date('Y-d-m H:i', strtotime($statiafor->tarigi))?>      </td>-->
         <td style="text-align:center;">  <?php echo $statiafor->avtori?>       </td> 
										   <td style="text-align:center; width:40px;"> <?php echo $statiafor->naxva?>        </td>
										    ...
										    <td style="text-align:center; width:80px;">  <?php echo   date('Y-d-m', strtotime($statiafor->tarigi))?>   </td>
